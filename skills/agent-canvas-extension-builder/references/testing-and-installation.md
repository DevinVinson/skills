# Testing and installing Agent Canvas Extensions
...
- extension directory and `canvas-extension.json` existence;
- valid JSON and supported `schema_version`;
- required fields and basic semantic version shape;
- kebab-case extension name, contribution IDs, and path segments;
...
Exercise the actual extension entrypoint in a DOM test environment. Keep the host double small and behavior-oriented.
...
The initial OpenHands frontend includes an MSW development mode that can install a checked-in fixture before Agent Server extension endpoints are available.
...
To test a separately located extension with this mock, copy it into a suitable fixture path only when authorized and avoid copying secrets-bearing files. Prefer adding a temporary ignored fixture or using the backend flow once available. Remove temporary copies after testing.

## Real Agent Server installation

Use Customize -> Extensions when available, or the authenticated management API when explicitly requested.
...
For a repository containing the extension at its root, omit `repo_path`. Pin or report the resolved revision returned by the backend.

### Multiple extensions in one repository

Place each extension under an independent package root:
...
Keep each manifest's `entrypoint` relative to that extension's package root. Configure shared build tooling to emit one self-contained browser ESM bundle per extension. Do not make one extension entrypoint import another extension's output at runtime; Blob URL loading prevents package-relative module resolution.
...
The Add extension form installs one extension per submission. It sends one `source`, optional `ref`, and optional `repo_path` in each install request. It does not scan a repository recursively, discover all `canvas-extension.json` files, or bulk-install nested extensions.
...
When preparing instructions for several extensions, generate an install matrix rather than saying to add the repository once. Keep the source/ref identical when appropriate and vary `repo_path` for each package root.

Do not upload or publish a local extension unless requested. Do not embed credentials in a source URL.
...
- Confirm the extension appears in Customize -> Extensions.
...
- Confirm no navigation item appears and no extension code executes while disabled.
...
- Confirm extension navigation respects the Canvas base path.
...
- Disable the extension and confirm all page registrations and navigation disappear.
- Re-enable and confirm the extension activates cleanly a second time.
- Update or reinstall when applicable and confirm stale effects do not remain.
- Switch backend or reconnect and confirm the previous backend's extension does not leak into the new context.

### Removal

- Uninstall only when explicitly requested.
- Confirm the inventory, navigation, and routes no longer expose the extension.

## Diagnosing common failures

### Extension is listed but has no page
...
Check that the extension is enabled and that `registerPage` uses an ID declared in the manifest.
...
### Extension works locally but not from Git
